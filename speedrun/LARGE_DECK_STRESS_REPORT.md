...
**Date:** 2026-07-01. **Deck:** `Cities_of_Your_Country.apkg` (218 MB, legacy
...
Both flagged issues are fixed; no test regressed (rust speedrun 18/18, qt
pgre+speedrecall 15/15, pylib speedrun 6/6, ruff clean, rustfmt clean).
...
| TopicMastery @50k (release, `just bench`) | p95 53 ms                         | **p95 32 ms**                    | one tag scan + one predicate note-tag pass instead of 9 per-subject scans (`rslib/src/speedrun/mod.rs`) |
...
- **TopicMastery scan:** replaced the 9 per-subject `tag:` searches (9 full
  ...
  its subject (hierarchical tag match). Correctness verified against the 18
  existing Rust tests (mastered counts, golden mean-R, coverage/abstain, etc.).
...
- The earlier 2/5 was an **intermittent, fully-recoverable** `fix_integrity`
  finding (it repairs on open), not corruption. Worth a longer run (e.g. 50
  rounds) to confirm it never escalates, but no data-loss path was observed.
...
- Re-run the crash test at 50 rounds to confirm the intermittent `fix_integrity`
  flag never becomes data loss.
